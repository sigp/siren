...
const IconButton:FC<IconButtonProps> = ({size, buttonType, icon, ...rest}) => {
  ...
  const containerClasses = addClassString('hover:scale-90 transition-all duration-100 ease-in cursor-pointer rounded-full flex items-center justify-center', [
    size ? size : 'w-8 h-8',
    getTypeClass()
  ])

  const iconClasses = addClassString('', [
    icon,
    getIconColor()
  ])

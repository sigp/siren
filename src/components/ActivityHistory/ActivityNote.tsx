...
const ActivityNote:FC<ActivityNoteProps> = ({activity, index, delayOffset, networkId, onHasSeen}) => {
        ...
        return (
          <Typography color="text-dark400" darkMode="dark:text-dark400" type="text-caption1">
            <Trans i18nKey="activityHistory.activities.deposit.text">
              <span className="underline font-bold">{{txHash: formatEthAddress(activityData.txHash)}}</span>
            </Trans>
          </Typography>
        )
      case ActivityType.IMPORT:
        ...
        <div className="flex flex-1 max-w-[500px] items-center space-x-6">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1.5">
              <Typography color="text-dark400" darkMode="dark:text-dark400" type="text-caption1">{text}</Typography>
              <Typography color="text-dark400" darkMode="dark:text-dark400" isBold type="text-caption1">{moment(createdAt).fromNow()}</Typography>
            </div>
          </div>
        </div>
        <div>
  ...
  return (
    <motion.div onMouseEnter={markAsSeen} initial={{x: 100, opacity: 0}} animate={{x: 0, opacity: 1}} transition={{duration: .2, delay: (.1 * (index || 0)) + (delayOffset || 0)}} className="p-4 border-style">
      {hasLink ? (

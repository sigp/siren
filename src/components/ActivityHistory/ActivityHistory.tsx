...
const ActivityHistory:FC<ActivityHistoryProps> = ({initActivityData, depositNetworkId}) => {
  ...
  const {data: streamedData} = useSSEData({url: '/activity-stream', isReady: true, isStateStore: true})
    ...
    <>
      ...
      >
        <div className="relative">
          {hasUnseenHistory && <div className="w-3 h-3 bg-error rounded-full absolute top-0 -right-1"/>}
      ...
      <AnimatePresence>
        {isOpenHistory && (
          <div className="fixed z-30 h-screen w-screen top-0 left-0">
            <motion.div initial={{opacity: 0}} animate={{opacity: .4}} exit={{opacity: 0, transition: {delay: .3}}} transition={{duration: .2}} className="w-full h-full bg-black" />
            <div className="absolute overflow-hidden top-0 right-0 h-screen w-1/2">
              <motion.div initial={{x: '100%'}} animate={{x: 0}}
                          transition={{duration: .3, type: 'spring', delay: .2}} exit={{x: '100%'}}
                          ref={ref} className="p-8 w-full h-full flex flex-col bg-white dark:bg-dark750">
                <div className="py-8 border-b-style mb-8">
                  <div onClick={closeHistory} className="flex items-center space-x-4 cursor-pointer">
                ...
                {chunkedHistory.length ? (
                  <FlexedOverflow isScrollAnim={isLoading} onScrollBottom={fetchNextPage} className="space-y-4">
                    {chunkedHistory.map(chunk => chunk.map((activity, index) => (
                      <ActivityNote key={index} networkId={depositNetworkId} onHasSeen={updateNoteHasSeen}
                                    delayOffset={isDelayOffset ? .2 : undefined} index={index} activity={activity}/>
                    )))}
                  </FlexedOverflow>
                ) : (
                  <div>

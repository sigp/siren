...
const ValidatorDepositRow:FC<ValidatorDepositRowProps> = ({candidate, mnemonic, beaconSpec, onDeposit, data}) => {
  const {t} = useTranslation()
  const { MIN_ACTIVATION_BALANCE } = beaconSpec
  const { index } = candidate
  const { isLoading, txHash, error, pubKey, keyStore, makeDeposit } = useValidatorDeposit({validator: candidate, mnemonic, beaconSpec})

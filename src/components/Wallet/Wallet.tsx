    ...
    <div ref={ref as any} className="z-10 relative">
      ...
      >
        <div className="relative">
          ...
          {!isValidNetwork && (
            <motion.div initial={{scale: .5}} animate={{scale: 1}} className="transition-all duration-300 ease-in w-4 h-4 bg-red-600 border border-white rounded-full absolute top-0 -right-1"/>
          )}
        </div>
        <div className="flex flex-1 items-center justify-between">
          ...
          <motion.div animate={isOpen ? 'up' : 'down'} initial="down" exit="down" variants={chevron}>
      ...
      {isOpen && (
        <div className="absolute w-[256px] top-full translate-y-1 left-1/2 -translate-x-1/2">
          <motion.div initial={{y: -80, opacity: 0}} animate={{y: 0, opacity: 1}} className="w-full bg-white dark:bg-dark750 shadow-2xl px-4 py-1">
              ...
              {isValidNetwork ? (
                <>
                  ...
                  <div className="flex space-x-2">
                    <Typography isBold type='text-caption1' color='text-dark500' darkMode="dark:text-dark100" family='font-roboto'>
                      {`${Number(formatted).toFixed(2)} ${symbol}`}
                    </Typography>
                    <Typography isBold type='text-caption1' color='text-dark300' darkMode="dark:text-dark500" family='font-roboto'>
                ...
                <div onClick={switchNetwork} className="w-full flex items-center space-x-3">
                 ...
                 <Typography type="text-caption1.5" color="text-error" darkMode="dark:text-error">{t('wrongNetwork')}</Typography>
                </div>
              )}
            </div>

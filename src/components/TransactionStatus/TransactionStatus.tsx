...
const TransactionStatus:FC<TransactionStatusProps> = ({title, id, text, children, txHash, status, networkId}) => {
  ...
  const iconContainerClass = addClassString('h-12 w-12 rounded-full flex items-center justify-center border', [getContainerClass()])
          ...
          {isValidNetwork ? (
            <Link href={etherScanLink} target="_blank">
              <div className="flex space-x-2 items-center">
                <Typography color="text-dark400" type="text-caption1" className="underline">{formatEthAddress(txHash)}</Typography>

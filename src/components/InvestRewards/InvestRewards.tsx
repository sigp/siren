...
const InvestRewards:FC<InvestRewardsProps> = ({candidateCount, rewardEstimate}) => {
  ...
  const timeLineOptions = [{title: t('week'), value: TimeUnit.WEEK}, {title: t('month'), value: TimeUnit.MONTH}, {title: t('year'), value: TimeUnit.YEAR}]

  return (
    <motion.div initial={{borderWidth: 0}} animate={{borderWidth: '1px'}} transition={{delay: .2}} className="w-full md:max-w-md border-style flex flex-col justify-around p-4">
      <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="space-y-8">
        <div className="flex justify-between items-center">
          <Typography color="text-dark400" darkMode="dark:text-dark400">{t('validatorManagement.investmentRewards.investment')}</Typography>
          <Typography color="text-dark300" darkMode="dark:text-dark300" type="text-subtitle2">{count}</Typography>
        </div>
        <div className="flex justify-between items-center">
          ...
          <Typography type="text-subtitle1" color="text-primary" darkMode="dark:text-primary" isBold>{stakeEthAmount} ETH</Typography>
        </div>
        <div className="flex justify-between items-center">
          <CurrencySelect rates={exchangeData} color='text-dark300' labelClass="xl:text-caption1" selection={currency} onSelect={selectCurrency} />
          <Typography color="text-dark500">{`${formattedPrefix} ${formatLocalCurrency(stakeEthRate)}`}</Typography>
        </div>
      </motion.div>
      <motion.hr initial={{width: 0}} animate={{width: '100%'}} transition={{delay: .2}} className="mx-auto h-px w-full border-style"/>
      <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="space-y-8">
        <div className="flex justify-between items-center">
          <Typography color="text-dark400" darkMode="dark:text-dark400">{t('validatorManagement.investmentRewards.rewards')}</Typography>
          <Typography color="text-dark300" darkMode="dark:text-dark300" type="text-subtitle2">{count}</Typography>
        </div>
        <div className="flex justify-between items-center">
          <div className="space-y-2">
          ...
          <div>
            <Typography type="text-subtitle2" color="text-primary" darkMode="dark:text-primary" isBold>+ {expectedReward} ETH</Typography>
            <Typography className="text-right" isCapitalize type="text-caption1" color="text-dark400" darkMode="dark:text-dark400">{t('validatorManagement.investmentRewards.perUnit', {unit: unit.toLowerCase()})}</Typography>
          </div>
        </div>
        <div className="flex justify-between items-center">
          <SelectDropDown

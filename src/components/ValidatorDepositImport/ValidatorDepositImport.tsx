...
const ValidatorDepositImport:FC<ValidatorDepositImportProps> = ({depositData, onRetryTx, onUpdateStatus, depositNetworkId}) => {
  ...
  const importValidator = async (keyStore) => {
    try {
      ...
      setImportError(true)
      displayToast(t('validatorManagement.signAndDeposit.unexpectedImportError', {txHash: shortHandTxHash, pubKey: shortHandPubKey}), ToastType.ERROR)
    }
  }
  ...
  const isValidNetwork = depositNetworkId === NetworkId.HOLESKY || depositNetworkId === NetworkId.MAINNET

  const beaconChaLink = isValidNetwork ? getBeaconChaLink(depositNetworkId, `/validator/${pubKey}`) : `http://127.0.0.1:64498/validator/${pubKey}`
  ...
  const renderTransactionStatus = () => {
    switch (true) {
      case isSuccess:
        return (
          <TransactionStatus
            id={mnemonicIndex} networkId={depositNetworkId}
            title={t('validatorManagement.txStatuses.validatorComplete.title')}
            status="success" txHash={txHash}>
            <div className="space-y-2">
              <Typography type="text-caption1">{t('validatorManagement.txStatuses.validatorComplete.text')}</Typography>
              <ExternalLink href={beaconChaLink} text={t('validatorManagement.reviewStatus', {pubKey: shortHandPubKey})}/>
            </div>
          </TransactionStatus>
        )
      case isImporting:
        return (
          <TransactionStatus id={mnemonicIndex} networkId={depositNetworkId}
                             title={t(`validatorManagement.txStatuses.${importError ? 'importError' : 'importPending'}.title`)}
                             text={!importError ? t(`validatorManagement.txStatuses.importPending.text`) : undefined}
                             status={importError ? 'error' : 'pending'} txHash={txHash}>
            {importError && (
              <div className="space-y-2">
                <Typography type="text-caption1">{t('validatorManagement.txStatuses.importError.text', {pubKey: shortHandPubKey})}</Typography>
                <ExternalLink href={beaconChaLink} text={t('validatorManagement.reviewStatus', {pubKey: shortHandPubKey})}/>
              </div>
            )}
          </TransactionStatus>
      ...
      default:
        return (
          <TransactionStatus id={mnemonicIndex} networkId={depositNetworkId}
             ...
             status={txStatus || 'pending'} txHash={txHash}>
            {txStatus === 'error' && (
              <div className="space-y-2">
  ...
  return (
    <div className="border-style rounded p-4">

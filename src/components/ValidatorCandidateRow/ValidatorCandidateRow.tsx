...
const ValidatorCandidateRow:FC<ValidatorCandidateRowProps> = ({data, onUpdateCandidate, index, children, isError}) => {
  const {t} = useTranslation()
  const {id, name} = data
  const valName = name || t('validatorManagement.customValidatorName')
  const updateCandidate = (e: ChangeEvent<HTMLInputElement>) => onUpdateCandidate?.(id, {...data, name: e.target.value})
  const containerClasses = addClassString('transition-all duration-50 group items-center w-full min-h-16 flex justify-between border-style border-t-0 relative', [isError && 'bg-lightError100'])

  return (
    <div className="w-full min-h-16 overflow-hidden">
      <motion.div initial={{translateY: -20}} animate={{translateY: 0}} className={containerClasses}>
        <div className="px-4 flex space-x-6 py-2 items-center">
          <div className="flex space-x-2 items-center">
            ...
            <div className="w-[180px] border-r relative">
              {
                onUpdateCandidate ? (
                  <InlineInput containerClass="w-full" inputClass="w-[90%]" value={name as string} onChange={updateCandidate}>
                    <div className="flex space-x-2 items-center relative">
                      <Typography className="group-hover:underline" type="text-caption1">{valName}</Typography>

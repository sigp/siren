...
export interface ValSetupTableProps extends Pick<ValSetupRowProps, 'onUpdateCandidate' | 'onRemoveCandidate'> {
...
const ValSetupTable:FC<ValSetupTableProps> = (props) => {
  const {candidates, onAddNewCandidate, onRemoveLastCandidate, onUpdateCandidate, onQuickSetCandidates, onRemoveCandidate} = props
      ...
      <div className="w-full border-style flex">
        ...
        <div className="group cursor-pointer flex border-l-style px-4 space-x-14 items-center">
          ...
          <div className="border-r px-4 mr-4">
            <InlineInput placeholder={String(count)} type="number" onChange={onQuickSetCandidates} inputClass="w-8">
              <div className="flex space-x-2 items-center">
                <i className="opacity-0 group-hover:opacity-100 bi-pencil-square text-dark900 dark:text-dark300"/>
                <Typography className="group-hover:underline" type="text-caption1">{count}</Typography>
              </div>
            </InlineInput>
          </div>
        </div>
        <div className="flex">
          <div className="p-4 border-l-style">
            <IconButton icon="bi-plus-circle" buttonType={IconButtonTypes.PRIMARY} onClick={onAddNewCandidate}/>
          </div>
          <div className="p-4 border-l-style">
            <IconButton icon="bi-dash-circle" buttonType={IconButtonTypes.SECONDARY} onClick={onRemoveLastCandidate}/>
          </div>
        </div>
      </div>
      <>
        {count > 0 ? (
          <div ref={divRef} className="max-h-[260px] overflow-scroll">
            {candidates.map((candidate, index) => (
              <ValSetupRow key={index} index={index} candidate={candidate} onRemoveCandidate={onRemoveCandidate} onUpdateCandidate={onUpdateCandidate}/>
            ))}
          </div>
        ) : (
          <InfoBox className="mt-4" type={InfoBoxType.NOTICE}>
            <div className="space-y-2">
              ...
              <div onClick={onAddNewCandidate} className="cursor-pointer">
                <Typography type="text-caption1" color="text-primary" darkMode="text-primary" className="underline">

...
const MnemonicIndexRow:FC<MnemonicIndexRowProps> = ({candidate, depositNetworkId}) => {
  ...
  const validClasses = addClassString('p-2 border rounded' ,[isPending ? 'border-style' : isValidIndex ? 'border-success bg-success100' : 'border-error bg-error100'])
  ...
  return (
    <ValidatorCandidateRow index={index} isValidate data={candidate} >
      <div className={containerClasses}>
        {pubKey && (
          <ExternalLink text={formatEthAddress(pubKey)} href={getBeaconChaLink(depositNetworkId, `/validator/${pubKey}`)} />
        )}
        <div className="px-4">
          <div className={validClasses}>
            <Typography color={isPending ? 'text-dark900' : isValidIndex ? 'text-success' : 'text-error'} isCapitalize
                        darkMode={isPending ? 'dark:text-dark400' : isValidIndex ? 'dark:text-success' : 'dark:text-error'}
                        className="whitespace-nowrap" type="text-caption1">

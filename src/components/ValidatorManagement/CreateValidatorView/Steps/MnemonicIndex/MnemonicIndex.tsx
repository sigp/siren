...
const MnemonicIndex:FC<MnemonicIndexProps> = ({candidates, keyPhrase, depositNetworkId, onNextStep, onBackStep, onValidatorChange, isActive}) => {
  ...
  const validateIndices = async () => {
    ...
    const processCandidate = async (candidate: ValidatorCandidate) => {
      ...
      try {
        ...
        return { ...candidate, isValidIndex: !Boolean(data.data), pubKey: publicKey, isPending: false };
      } catch (e) {
  ...
  const isDisabledVerify = startIndex === undefined || startIndex < 0 || (startIndex + candidates.length) > 4294967295
  const invalidCount = isLoading ? 0 : indexedValidatorCandidates.filter(({isValidIndex}) => !isValidIndex).length
  ...
  return (
    <div className="w-full h-full relative space-y-4">
      <div>
        <Typography type="text-caption1">{t('validatorManagement.mnemonicIndexing.title')} --</Typography>
        <Typography type="text-subtitle2" fontWeight="font-light">{t('validatorManagement.mnemonicIndexing.subTitle')}</Typography>
      </div>
      <div className="w-full max-w-[650px] space-y-8">
        <Typography type="text-caption1">{t('validatorManagement.mnemonicIndexing.caption')}</Typography>
        <div className="flex w-full">
          <input ref={inputRef} onChange={setStartIndex} min={0} max={4294967295} onKeyDown={handleEnterKey}
                 className="w-full text-dark900 dark:text-dark300 dark:bg-dark600_20 font-openSauce text-caption1 p-2 outline-none border-style" type="number" />
          <Button isLoading={isLoading} isDisabled={isDisabledVerify} onClick={validateIndices} type={ButtonFace.SECONDARY}>{t('verify')}</Button>
        </div>
            ...
            <>
              <div className="w-full flex justify-end border border-style px-2 py-3">
                ...
                <div className="flex space-x-2 px-2">
                  <Typography isCapitalize type="text-caption1">{t('inUse')}: </Typography>
              ...
              <div className="overflow-scroll w-full border-b-style max-h-[190px]">
                {indexedValidatorCandidates.map((candidate, index) => (
                    <MnemonicIndexRow depositNetworkId={depositNetworkId} key={index} candidate={candidate}/>
                  ))}
              </div>
            </>
          ) : (
        ...
        <StepOptions onBackStep={stepBack} onNextStep={stepForward} isDisabledNext={!count || invalidCount > 0}/>
      </div>
    </div>
  )
}

...
const KeystoreAuthRow:FC<KeystoreAuthRowProps> = ({candidate, index, onConfirmation}) => {
  ...
  const storeConfirmationPassword = (e: ChangeEvent<HTMLInputElement>) => setConfirmationPassword(e.target.value)
  ...
  return (
    <ValidatorCandidateRow isError={Boolean(errorMessages.length) || !isMatchingPassword } data={candidate} index={index !== undefined ? Number(index) : undefined}>
      <div className="p-8 flex-1 space-y-8">
        <div className="relative">
          {isValid && <i className="bi bi-check-circle text-success text-caption absolute top-2 -left-5"/>}
          <Input placeholder={t('password')} error={errorMessages.length ? errorMessages.join(' • ') : undefined} className="text-caption1" inputStyle="secondary" type="password" onChange={storePassword}/>
        </div>
        <div className="relative">
          {isValid && <i className="bi bi-check-circle text-success text-caption absolute top-2 -left-5"/>}
          <Input placeholder={t('confirmPassword')} error={!isMatchingPassword ? t('error.passwordMatch') : undefined} inputStyle="secondary" type="password" className="text-caption1"  onChange={storeConfirmationPassword}/>
        </div>
      </div>

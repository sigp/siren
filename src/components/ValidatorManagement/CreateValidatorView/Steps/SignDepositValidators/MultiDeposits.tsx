...
const MultiDeposits:FC<MultiDepositsProps> = ({candidates, mnemonic, sharedWithdrawalCredentials, sharedKeystorePassword, beaconSpec}) => {
  ...
  const storeDepositInfo = (txHash: string, keyStore: any, pubKey: string, mnemonicIndex: number) => {
    setDepositData(prev => ([...prev, {txHash, keyStore, pubKey, mnemonicIndex, status: 'pending'}]))
  }
            ...
            <>
              ...
              <FlexedOverflow className="border-b-style">
                {candidates.map((validator, index) => (
                  <ValidatorDepositRow key={index} data={depositData.find(data => data.mnemonicIndex === validator.index)}
                                       beaconSpec={beaconSpec}
                                       onDeposit={storeDepositInfo} mnemonic={mnemonic}
                                       candidate={{ ...validator, keyStorePassword: sharedKeystorePassword, withdrawalCredentials: sharedWithdrawalCredentials }}/>
                ))}
              </FlexedOverflow>
            </>
          ) : (
        ...
        {depositData.length ? (
          <FlexedOverflow isAutoScroll className="space-y-3 pt-3">
            {depositData.map((data, index) => (
              <ValidatorDepositImport key={index} onUpdateStatus={updateDepositInfoStatus}
                                      onRetryTx={removeTransaction} depositData={data} depositNetworkId={DEPOSIT_NETWORK_ID}  />
            ))}
          </FlexedOverflow>
        ) : (

...
const SingleDeposit:FC<SingleDepositProps> = ({candidate, mnemonic, onComplete, beaconSpec, ...props}) => {
  ...
  const { isLoading, error, keyStore, txHash, pubKey, makeDeposit } = useValidatorDeposit({validator: candidate, mnemonic, beaconSpec })
    ...
    <div className="relative w-full h-full">
      <div className="flex pt-8 w-full h-full">
        <div className="flex-1 space-y-8">
          <div>
            <Typography type="text-caption1">{t('validatorManagement.signAndDeposit.title')} --</Typography>
            <Typography type="text-subtitle2" fontWeight="font-light">{t('validatorManagement.signAndDeposit.subTitle')}</Typography>
          </div>
          <div className="w-[80%] pt-6 flex">
            {isAcknowledgeRisk && (
              <div className="flex-1 space-y-8">
                ...
                <div>
                  <Typography type="text-subtitle2">{name || t('validatorManagement.customValidatorName')}</Typography>
                  {withdrawalCredentials && <Typography color="text-dark400" type="text-subtitle3">{formatEthAddress(withdrawalCredentials)}</Typography>}
                  <Typography color="text-dark400" type="text-subtitle3">{index}</Typography>
                </div>
              </div>
            )}
            {isAcknowledgeRisk ? (
              <>
                {isSuccessScreen ? (
                  <ValidatorSuccessScreen validatorPubKey={pubKey} networkId={DEPOSIT_NETWORK_ID} onClick={onComplete}/>
                ) : (
                  <div className="w-[500px] shadow">
                    <VerticalStepper step={step} titles={stepTitles}>
                      <DepositStep depositAmount={MIN_ACTIVATION_BALANCE} isLoading={isLoading} onDeposit={makeDeposit}/>
                      <ValidateTransactionStep txHash={txHash} txStatus={txStatus} onRetry={retryTransaction} networkId={DEPOSIT_NETWORK_ID} />
                      <ImportValidatorStep txHash={txHash} networkId={DEPOSIT_NETWORK_ID} pubKey={pubKey} isImportError={isImportError} />
                    </VerticalStepper>
                  </div>
                )}
              </>

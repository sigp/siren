  ...
  return (
    <div className="py-4 space-y-2">
      <InfoBox type={InfoBoxType.NOTICE}>
        <Typography darkMode="text-dark900" color="text-dark900" type="text-caption1">{t('validatorManagement.signAndDeposit.depositText')}</Typography>
      </InfoBox>
      <WalletActionBtn isSufficientBalance={isSufficient}>
        <Button isLoading={isLoading} type={ButtonFace.SECONDARY} className="w-full" onClick={onDeposit}>{t('validatorManagement.makeDeposit')}</Button>
      </WalletActionBtn>
    </div>
  )
}

  ...
  return (
    <RodalModal onClose={onClose} styles={{ maxWidth: '450px' }} isVisible={isOpen}>
      <div className="w-full h-full flex flex-col p-6 space-y-6 items-center justify-center">
        ...
        <Typography type="text-caption1" className="text-center">{t('validatorManagement.credentialRiskModal.text')}</Typography>
        <Button onClick={onAccept} type={ButtonFace.TERTIARY}>{t('understandAndAccept')}</Button>
      </div>
    </RodalModal>
  )
}

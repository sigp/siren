...
const CreateValidatorView:FC<CreateValidatorViewProps> = ({validatorNetworkData, onChangeView}) => {
    ...
    <div className="flex-1">
      <div className="w-full">
        <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="w-full flex">
          {steps.map((step, index) => (
            <div key={index} className="flex-1 h-11 bg-dark25 dark:bg-dark750 flex items-center justify-center border-r dark:border-r-dark600 last:border-r-0">
      ...
      <div className="w-full h-full relative createSlide">
        <Carousel swiping={false} slideIndex={step} dragging={false} withoutControls>
          <CreateValidatorStep rewardEstimate={calculatedRewards} candidateCount={totalCandidates}>
            <ValidatorSetup onContinue={incrementStep} onValidatorChange={setNewValidators} candidates={candidates}/>
          </CreateValidatorStep>

          <CreateValidatorStep rewardEstimate={calculatedRewards} candidateCount={totalCandidates}>
            <MnemonicPhrase isActive={step === 1} onNextStep={incrementStep} value={keyPhrase} onChange={setPhrase} onBackStep={decrementStep}/>
          </CreateValidatorStep>

          {DEPOSIT_NETWORK_ID && (
            <CreateValidatorStep rewardEstimate={calculatedRewards} candidateCount={totalCandidates}>
              <MnemonicIndex depositNetworkId={DEPOSIT_NETWORK_ID} isActive={step === 2} onBackStep={decrementStep} onValidatorChange={setNewValidators} onNextStep={incrementStep} keyPhrase={keyPhrase} candidates={candidates}/>
            </CreateValidatorStep>
          )}

          <CreateValidatorStep rewardEstimate={calculatedRewards} candidateCount={totalCandidates}>
            <WithdrawalCredentials  isConnected={isConnected} onShowRisk={showRiskMessage}
                  isActive={step === 3} candidates={candidates}
                  onValidatorChange={setNewValidators}  onBackStep={decrementStep} onNextStep={incrementStep}
                  onUpdateSharedCredentials={setCredentials} sharedCredentials={sharedWithdrawalCredentials}
            />
          </CreateValidatorStep>

          <CreateValidatorStep rewardEstimate={calculatedRewards} candidateCount={totalCandidates}>
            <KeystoreAuthentication onUpdateCandidates={setNewValidators} sharedKeystorePassword={sharedKeystorePassword} setSharedKeystorePassword={setKeystorePassword} candidates={candidates} onBackStep={decrementStep} onNextStep={incrementStep}/>
          </CreateValidatorStep>

          {Boolean(candidates.length) && beaconSpec && (
            <SignDeposit sharedKeystorePassword={sharedKeystorePassword} beaconSpec={beaconSpec} sharedWithdrawalCredentials={sharedWithdrawalCredentials} onComplete={viewManagement}
                         rewardEstimate={calculatedRewards} candidateCount={totalCandidates} mnemonic={keyPhrase} candidates={candidates}/>
          )}
        </Carousel>
      </div>
    </div>

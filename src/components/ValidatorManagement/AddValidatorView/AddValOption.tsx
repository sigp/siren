...
const AddValOption:FC<AddValOptionProps> = ({data, onChangeView, animControls, index}) => {
  ...
  const containerClass = addClassString('flex-1 max-w-80 h-[409px]', [isDisabled && 'opacity-15 dark:opacity-40 pointer-events-none'])
  return (
    <div className={containerClass}>
      <AddValidatorAction onClick={() => onChangeView(view)}  animate={animControls} initial={{x: 40, opacity: 0}}
                          isRecommended={isRecommended} index={index} title={title} subTitle={subTitle} caption={caption}>

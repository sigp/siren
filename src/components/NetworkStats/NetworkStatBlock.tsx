...
const NetworkStatBlock: FC<NetworkStatBlockProps> = ({
  ...
  return (
    <div className={`${className} py-4 px-4 md:py-2 md:px-2 xl:px-4 h-full flex flex-col justify-between w-full space-y-6 md:space-y-0 border-b-style500 md:border-b-0 md:w-40 @1600:w-auto @1600:flex-1 md:border-r-style500`}>
      <Typography type='text-tiny' className='uppercase' isBold darkMode='dark:text-white'>
        {title}
      </Typography>
      <div className={`flex items-center justify-between md:justify-start md:space-x-4 ${status ? 'justify-between' : ''}`}>
        {status ? (
